...
// src/static/chatLayout.ts
var initialWebviewContext = (extensionUri, panel, currentChat) => {
  ...
  panel.webview.html = `<!DOCTYPE html>
      ...
      <body id="app">
          ...
          <nav class="chat-nav">
              ...
              <button id="old-chat-btn" class="nav-button">\u{1F551}</button>
          </nav>
          <div id="main-container">
            ...
            <div class="prompt-container">
              ...
              <div class="prompt-btns">
                  ...
                  <button id="attachment" class="prompt-btn">\u{1F4CE}</button>
              </div>
            </div>
          </div>
...
var App = class _App {
  constructor(extensionUri, anthropic, chatStorage, currentChat) {
    ...
    this._panel = vscode2.window.createWebviewPanel(
      ...
      vscode2.ViewColumn.Beside,
      {
        ...
        localResourceRoots: [
          vscode2.Uri.joinPath(extensionUri, "src"),
          vscode2.Uri.joinPath(extensionUri, "out")
        ]
      }
    );
  ...
  static async createOrShow(extensionUri, chatStorage) {
    const config = vscode2.workspace.getConfiguration("claudeAI");
    const apiKey = config.get("apiKey");
    if (!apiKey) {
      vscode2.window.showErrorMessage("Please set your Claude API key in settings");
      return;
    }
    const anthropic = new sdk_default({ apiKey });
    if (_App._current) {
      _App._current._panel.reveal(vscode2.ViewColumn.Beside);
      return;
    }
    let currentChat;
    try {
    ...
    _App._current = new _App(extensionUri, anthropic, chatStorage, currentChat);
  }
...
You're viewing a code project in VSCode. The user's current context is a Claude extension for VSCode.`;
      ...
      if (allMessages.length > 1) {
        ...
        const MAX_TOKENS_ESTIMATE = 4e3;
        const RECENCY_THRESHOLD = 3;
        ...
        const relevantMessages = allMessages.slice(1, -RECENCY_THRESHOLD).filter((msg) => {
      ...
      const previousMessages = contextMessages.map((message) => {
        if (message.attachments && message.attachments.length > 0) {
          return {
            role: message.role,
            content: message.content + "\n(File attachment: " + message.attachments.map((a2) => a2.fileName).join(", ") + ")"
          };
        }
        return {
          role: message.role,
...
Note: Some earlier messages in this conversation have been omitted for context management. The history provided is a subset of the complete conversation.`;
      ...
      vscode2.window.showErrorMessage("Error communicating with Claude: " + error);
      this._panel.webview.postMessage({
        command: "receiveMessage",
  ...
  async _handleCreateNewChat() {
    try {
      ...
      vscode2.window.showErrorMessage("Error creating new chat: " + error);
    }
  }
  async _handleGetAllChats() {
    try {
      ...
      vscode2.window.showErrorMessage("Error loading chats: " + error);
    }
  }
  async _handleLoadChat(chatId) {
    try {
      ...
      if (chatToLoad) {
        ...
        vscode2.window.showErrorMessage("Chat not found");
      }
    } catch (error) {
      console.error("Error loading chat:", error);
      vscode2.window.showErrorMessage("Error loading chat: " + error);
    }
  }
  async _handleDeleteChat(chatId) {
    ...
    if (success) {
      ...
      vscode2.window.showErrorMessage("Failed to delete chat");
    }
  }
  ...
  async _handleFileAttachment(fileName, fileContent, fileType) {
    try {
      ...
      if (!this._attachmentStorage) {
        this._attachmentStorage = /* @__PURE__ */ new Map();
      }
      ...
      if (fileType.includes("text") || fileType.includes("javascript") || fileType.includes("typescript") || fileType.includes("json") || fileType.includes("html") || fileType.includes("css")) {
...
Please analyze this file and provide insights.`;
      ...
      vscode2.window.showErrorMessage("Error processing file: " + error);
    }
  }
  _onDispose() {
    _App._current = void 0;
